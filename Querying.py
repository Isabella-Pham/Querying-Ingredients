...
def getFrequencies(data): #determines the frequencies of each food group
    ...
    for food in data:
        for info in food:
            if info == 'group':
                freqs = increaseFreq(freqs, food[info])
                groups.append(food[info])
    return freqs

def increaseFreq(freqs, group): #takes in a food group and adds it to dictionary
    for key in freqs:
        if group==key:
            freqs[key] += 1
    return freqs

def getHighestFreq(freqs): #takes in dictionary of freqs and returns most common group
...
def entropy(freqs): #determines whether or not the entropy is large enough
    ...
    for group in freqs:
        if abs(freqs[group] - highest) < 1 and group != getHighestFreq(freqs):
